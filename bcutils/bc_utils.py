...
async def _login_async(human: Humanization, username: str, password: str) -> None:
    ...
    if await login_link.count() == 0 and (
        home_response is None or home_response.status == 202
    ):
        logger.info("step: wait for LOGIN link")
        try:
            await login_link.wait_for(state="visible", timeout=10000)
        except PlaywrightTimeoutError:
            logger.info("step: LOGIN link did not appear")
    ...
    if await email_box.count() == 0:
        if human.page.url.rstrip("/") == (BARCHART_URL + "login").rstrip("/"):
            raise BCException("Unable to verify Barchart authentication state")
        logger.info("step: persisted login verified")
        return

    logger.info("step: fill email")
    await email_box.fill("")
    await human.type_at(email_box, username)
    if await email_box.input_value() != username:
        logger.info("step: correct humanized email input")
        await email_box.fill(username)
    await _pause(human)

    password_box = human.page.get_by_role("textbox", name="Password")
    if await password_box.count() == 0:
        raise BCException("Unable to locate Barchart password field")
    ...
    if await password_box.input_value() != password:
        logger.info("step: correct humanized password input")
        await password_box.fill(password)
    await _pause(human)

    login_button = human.page.get_by_role("button", name="Login")
    if await login_button.count() == 0:
        login_button = human.page.locator("button[type=submit]").first
    if await login_button.count() == 0:
        raise BCException("Unable to locate Barchart login button")
...
async def _update_barchart_contract_file_async(
    ...
    if res == Resolution.Hour:
        ...
        start = last_index_date + timedelta(days=1)
...
def _raw_barchart_data_to_df(
    ...
    return BarchartPriceNormalizer.normalize(df)
